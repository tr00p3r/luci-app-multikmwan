...
- A transfer that fails (a stalled stream, a server hiccup) is retried once after two seconds, per direction. If the run still fails, the link keeps its last good result for up to six hours instead of dropping out of the ranking, the failed attempt stays in the speed-test log, and the Status card says it is showing the previous result. Auto-rank never demotes the current leader on a round in which it has no score.
- The Status page shows a running speed or website test in place: which link, which step, and "2 of 3"; the link's card is tagged while it is being tested and the test buttons are disabled until the run ends.
